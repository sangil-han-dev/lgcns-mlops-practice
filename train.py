...
if __name__ == "__main__":
    csv_filepath = os.path.join(DATA_PATH, "house_rent_train.csv")
    train_df = pd.read_csv(csv_filepath)
    ...
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            logger.info(
                f"Cross-Validation RMSE score for the current run"
                f"{run_name} : {score_cv.mean():.4f} (std = {score_cv.std():.4f})"
            )
